...
BITSADMIN /TRANSFER "%CLOUDCOINMANAGERPORTABLE_name% Update Download" /DOWNLOAD /DYNAMIC "https://github.com/BigRedBrent/CloudCoinManagerPortable/raw/main/version.txt" "%CD%\version.tmp" > NUL 2>&1
BITSADMIN /CANCEL "%CLOUDCOINMANAGERPORTABLE_name% Update Download" > NUL 2>&1
FOR /F "tokens=*" %%G IN ('BITSADMIN /LIST 1^>NUL') DO CALL :version_remove_job "%%G"
GOTO version_skip_remove_job
:version_remove_job
SET CLOUDCOINMANAGERPORTABLE_version_guid=%~1
ECHO %~1 | FIND "%CLOUDCOINMANAGERPORTABLE_name% Update Download" > NUL 2>&1 && BITSADMIN /CANCEL %CLOUDCOINMANAGERPORTABLE_version_guid:~0,38% > NUL 2>&1
EXIT /B
